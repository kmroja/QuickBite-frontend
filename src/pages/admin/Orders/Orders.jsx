...
import { navLinksSidebar } from "../../../assets/dummyAdmin";
...
const Orders = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const res = await axios.get("https://quickbite-backend-6dvr.onrender.com/api/orders/getall", {
  ...
  if (loading) return <div className={layoutClasses.page + " flex items-center justify-center text-amber-400"}>Loading orders...</div>;
  if (error) return <div className={layoutClasses.page + " flex items-center justify-center text-red-400"}>{error}</div>;
                  ...
                  {orders.map(order => {
                    ...
                    return (
                      <tr key={order._id} className={tableClasses.row}>
                        ...
                        <td className={tableClasses.cellBase}>{order.address}, {order.city} - {order.zipCode}</td>
                        <td className={tableClasses.cellBase}>
                          {order.items.map((itm, idx) => (
                            <div key={idx} className="flex items-center gap-3 p-2 rounded-lg">
                              ...
                              <div className="flex-1 text-amber-100/80 text-sm">{itm.item.name} x{itm.quantity}</div>
                            </div>
                          ))}
                        </td>
                        <td className={tableClasses.cellBase + " text-center"}>{totalItems}</td>
                        ...
                        <td className={tableClasses.cellBase}>
                          <select value={order.status} onChange={e => handleStatusChange(order._id, e.target.value)}
                            ...
                            {Object.keys(statusStyles).map(s => <option key={s} value={s}>{statusStyles[s].label}</option>)}
                          </select>
                        </td>
                      </tr>
                    );
