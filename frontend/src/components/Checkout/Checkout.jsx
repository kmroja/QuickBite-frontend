...
const CheckoutPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#1e2d20] to-[#2e3f2b] text-white py-16 px-4">
  <div className="max-w-4xl mx-auto">
    <Link to="/cart" className="flex items-center gap-2 text-lime-300 mb-8">
    ...
    <form onSubmit={handleSubmit} className="grid lg:grid-cols-2 gap-12">
      ...
      <div className="bg-[#3e563c]/80 p-6 rounded-3xl space-y-6">
      ...
      <div className="bg-[#3e563c]/80 p-6 rounded-3xl space-y-6">
        ...
        <div className="space-y-4 mb-6">
          <h3 className="text-lg font-semibold text-lime-100">Your Order Items</h3>
          {cartItems.map(({ _id, item, quantity }) => (
            <div key={_id} className="flex justify-between items-center bg-[#2e3f2b]/60 p-3 rounded-lg">
              <div className="flex-1">
                <span className="text-lime-100">{item.name}</span>
                <span className="ml-2 text-lime-400/80 text-sm">x{quantity}</span>
              </div>
              <span className="text-lime-300">₹{(item.price * quantity).toFixed(2)}</span>
            </div>
          ))}
        </div>
        ...
        <div>
          ...
          <select
            ...
            required
            className="w-full bg-[#2e3f2b]/60 rounded-xl px-4 py-3"
          >
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-gradient-to-r from-lime-700 to-lime-500 py-4 rounded-xl font-bold flex justify-center items-center"
        >
